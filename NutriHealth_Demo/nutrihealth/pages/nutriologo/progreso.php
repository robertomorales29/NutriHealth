    ...
    <div class="row g-4">
        <div class="col-lg-4">
            <div class="card card-soft p-4 mb-4">
                ...
                <form method="get">
                    <select name="paciente_id" class="form-select mb-3" onchange="this.form.submit()">
                        <?php foreach ($patients as $p): ?><option value="<?= (int)$p['id'] ?>" <?= $pacienteId === (int)$p['id'] ? 'selected' : '' ?>><?= e(full_name($p)) ?></option><?php endforeach; ?>
                    </select>
            ...
            <div class="card card-soft p-4">
                ...
                <form method="post" action="<?= base_url('actions/historial_guardar.php') ?>">
                    ...
                    <div class="mb-3"><label class="form-label">Masa muscular</label><input type="number" step="0.01" name="masa_muscular" class="form-control"></div>
                    <div class="row g-2"><div class="col-6 mb-3"><label class="form-label">Cintura</label><input type="number" step="0.01" name="cintura" class="form-control"></div><div class="col-6 mb-3"><label class="form-label">Cadera</label><input type="number" step="0.01" name="cadera" class="form-control"></div></div>
        ...
        <div class="col-lg-8">
            <div class="card card-soft p-4">
                ...
                <?php if ($health): ?><div class="alert alert-info">Objetivo: <?= e($health['objetivo_salud']) ?> · Actividad: <?= e($health['nivel_actividad']) ?> · Alergias: <?= e($health['alergias']) ?></div><?php endif; ?>
                <div class="table-responsive"><table class="table align-middle"><thead><tr><th>Fecha</th><th>Peso</th><th>% grasa</th><th>Masa muscular</th><th>Observaciones</th></tr></thead><tbody>
                <?php if (!$historial): ?><tr><td colspan="5" class="text-muted">Sin historial.</td></tr><?php endif; ?>
                <?php foreach ($historial as $h): ?><tr><td><?= e($h['fecha_registro']) ?></td><td><?= e($h['peso']) ?></td><td><?= e($h['porcentaje_grasa']) ?></td><td><?= e($h['masa_muscular']) ?></td><td><?= e($h['observaciones']) ?></td></tr><?php endforeach; ?>
                </tbody></table></div>
            </div>
        </div>
    </div>
